...
pickle_filepath='/home/shikhar/Desktop/full run/error.pickle'
...
def error_func(pred_image,lab_images):
    x_1=pred_image[:,:,:,1]
    x_2=pred_image[:,:,:,2]
    y_1=lab_images[:,:,:,1]
    y_2=lab_images[:,:,:,2]
    ...
    x_1=np.array(x_1)
    x_1=np.array(x_2)
    z1=tf.reduce_mean(tf.sum(x_1*x_2))
    z2=tf.reduce_mean(tf.sum(y_1*y_2))
    z=(-1)*(z2)*tf.log(z1)
    return z
